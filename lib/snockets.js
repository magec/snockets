...
(function() {
  ...
  module.exports = Snockets = (function() {
    function Snockets(options) {
      ...
      if ((_base = this.options).src == null) {
        _base.src = '.';
      }
      if ((_base1 = this.options).async == null) {
        _base1.async = true;
      }
      if ((_base2 = this.options).sort == null) {
        _base2.sort = false;
      }
      this.cache = {};
      this.concatCache = {};
    ...
    Snockets.prototype.getConcatenation = function(filePath, flags, callback) {
      ...
      return this.updateDirectives(filePath, flags, function(err, graphChanged) {
        var chain, concatenation, e, link, result, _ref, _ref1;
        if (err) {
          if (callback) {
            return callback(err);
          } else {
        ...
        if (flags.minify) {
          if ((_ref1 = _this.concatCache[filePath]) != null ? _ref1.minifiedData : void 0) {
    ...
    Snockets.prototype.readdir = function(dir, flags, callback) {
      var e, files;
      if (flags.async) {
        return fs.readdir(this.absPath(dir), function(err, files) {
          return callback(err, flags.sort ? files.sort() : files);
        });
      } else {
        try {
          files = fs.readdirSync(this.absPath(dir));
          return callback(null, flags.sort ? files.sort() : files);
        } catch (_error) {
          e = _error;
          return callback(e);
        }
